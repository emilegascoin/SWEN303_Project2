                    ...
                    <IonList>
                        { cartProducts && cartProducts.map((product, index) => {
                            if ((index <= amountLoaded)) {
                                return (
                                <IonItem className={ styles.cartSlider }>
                                    <IonItemOption color = "transparent" key={ index } lines="none" detail={ false } className={ styles.cartItem } routerLink={ `/category/${ product.category.slug }/${ product.product.id }`}>
                                    ...
                                    <div slot = "end" className={ styles.cartActions }>
                                        <IonBadge color="dark">{ product.product.price }</IonBadge>
                                    </div>

                                    <IonButton color="light" style={{ width: "4rem", height: "4rem" }} onClick={ () => removeProductFromCart(index)} slot = "end">
